(() => {
    ...
    const fullpage = document.querySelector("[data-fp]");
    if (fullpage) modules_flsModules.fullpage = new FullPage(fullpage, "");
    class ScrollWatcher {
        constructor(props) {
            let defaultConfig = {
                logging: true
            };
            this.selector = document.querySelectorAll("[data-watch]");
        ...
        scrollWatcherRun() {
            ...
            this.scrollWatcherConstructor(this.selector);
        }
        scrollWatcherConstructor(items) {
            if (items.length) {
                ...
                let uniqParams = uniqArray(Array.from(items).map((item => {
                    ...
                    return `${watchRoot}|${watchMargin}|${watchThreshold}`;
                })));
                uniqParams.forEach((uniqParam => {
                    ...
                    let groupItems = Array.from(items).filter((item => {
                        let watchRoot = item.dataset.watchRoot ?? null;
                        let watchMargin = item.dataset.watchMargin ?? "0px";
                        let watchThreshold = item.dataset.watchThreshold ?? 0;
        ...
        getScrollWatcherConfig(paramsWatch) {
            let configWatcher = {};
            const parentRoot = document.querySelector(paramsWatch.root);
            if (parentRoot) configWatcher.root = parentRoot; else if (paramsWatch.root !== "null") this.scrollWatcherLogging(`Эмм... батьківського об'єкта ${paramsWatch.root} немає на сторінці`);
